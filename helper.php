...
/**
 * Helper for mod_tags_related
 *
 * @since  3.1
 */
abstract class ModTagsrelatedHelper
{
	/**
	 * Get a list of tags
	 *
	...
	public static function getList(&$params)
	{
		...
		$query->where('cc.core_catid IN (' . $category . ')');
		...
		if ($ordering === 'published_asc')
		{
			$query->order($db->quoteName('core_publish_up') . ' ASC');
		}

		if ($ordering === 'published_desc')
		{
			$query->order($db->quoteName('core_publish_up') . ' DEsC');
		}

		if ($ordering === 'created_asc')
		{
			$query->order($db->quoteName('core_created_time') . ' ASC');
		}

		if ($ordering === 'created_desc')
		{
			$query->order($db->quoteName('core_created_time') . ' DEC');
		}
